...
def df_operativa(spark):
    ...
    return spark.createDataFrame(
        data, ["id_cliente", "fecha_saldo",
               "codigo_cliente", "importe_operativa"])
...
def df_clientes(spark):
    ...
    data = [
        ("A", "Ana", "Gómez", "López",
         ...
         "12345678A"),
        ("B", "Luis", "Pérez", "Martínez",
         "Calle 2",
         datetime.strptime("1980-06-15", "%Y-%m-%d"),
         "87654321B"),
    ]
...
def test_max_saldo_por_cliente(df_operativa):
    ...
    assert sorted([
        (r.id_cliente, r.max_saldo) for r in result
    ]) == sorted(expected)
...
def test_fecha_mas_reciente_por_operacion(df_operativa):
    ...
    result = df_result.select("id_cliente",
                              "codigo_cliente",
                              "fecha_mas_reciente").distinct().collect()
    expected = [
        (1, "A", "2023-01-02"),
        (2, "B", "2023-01-01"),
    ]
    assert sorted([
        (r.id_cliente, r.codigo_cliente, r.fecha_mas_reciente) for r in result
    ]) == sorted(expected)
...
def test_calcular_saldo_final(df_operativa):
    ...
    result = df_result.select(
        "codigo_cliente", "saldo_final").distinct().collect()
    expected = [("A", 300.00), ("B", 50.00)]
    assert sorted([
        (r.codigo_cliente, r.saldo_final) for r in result
    ]) == sorted(expected)
...
def test_informacion_cliente(df_operativa, df_clientes):
    ...
    result = df_result.select(
        "codigo_cliente",
        "nombre_completo",
        "menor_30").orderBy("codigo_cliente").collect()
    ...
    assert [
        (r.codigo_cliente, r.nombre_completo, r.menor_30)
        for r in result
    ] == expected
...
def test_run(df_operativa, df_clientes):
    ...
    result = df_result.select(
        ...
        "nombre_completo",
        "menor_30"
    ).orderBy("codigo_cliente").collect()
