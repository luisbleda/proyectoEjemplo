...
def test_max_saldo_por_cliente(df_operativa):
    ...
    pd.testing.assert_frame_equal(
        df_result.reset_index(drop=True), expected)
...
def test_fecha_mas_reciente_por_operacion(df_operativa):
    ...
    expected_fechas = ["2023-01-02",
                       "2023-01-02",
                       "2023-01-01"]
    assert list(
        df_result["fecha_mas_reciente"]) == expected_fechas
...
def test_calcular_saldo_final(df_operativa):
    ...
    for _, row in df_result.iterrows():
        assert round(
            row["saldo_final"], 2) == expected_saldos[
            row["codigo_cliente"]]
...
def test_informacion_cliente(df_operativa, df_clientes):
    ...
    assert df_result.loc[
               df_result["codigo_cliente"] == "A", "nombre_completo"
           ].values[0] == "Ana Gómez López"
    ...
    assert df_result.loc[
               df_result["codigo_cliente"] == "A", "menor_30"
           ].values[0] == esperado
